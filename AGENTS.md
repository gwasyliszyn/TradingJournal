...
Astro 6 SSR app (`output: "server"`) with React 19 islands, Tailwind CSS 4, Supabase auth, shadcn/ui components. Deployed to Cloudflare Workers via `@astrojs/cloudflare` adapter.

**Domain:** Trading discipline journal — traders complete a daily loop: pre-market check-in → session plan → trade logging → post-session review → Process Score. See `@context/foundation/prd.md` for full requirements.

### Request lifecycle

1. Every request hits `src/middleware.ts` which creates a Supabase SSR client, resolves the current user from cookies, and attaches it to `context.locals.user`.
2. Routes listed in `PROTECTED_ROUTES` (in middleware.ts) redirect to `/auth/signin` if unauthenticated.
3. Astro pages render server-side; React components hydrate as islands.

### Auth

- `src/lib/supabase.ts` — SSR client factory using `@supabase/ssr` with cookie-based sessions.
- Env vars `SUPABASE_URL` and `SUPABASE_KEY` are server-only secrets declared via `astro:env` schema in `astro.config.mjs`. Import from `astro:env/server`, never expose to client.
...
- **Supabase migrations**: `supabase/migrations/` with naming format `YYYYMMDDHHmmss_short_description.sql`. Always enable RLS on new tables.
...
Each screen presents one primary action. No secondary actions, sidebars, or decorative elements on form views. Multi-step flows use progressive disclosure — one form section at a time, never scroll-to-fill.

### Environment & CI

See `@README.md` for environment setup, local Supabase, and CI configuration.
